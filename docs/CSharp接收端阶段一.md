...
- **顶栏两行**
  - 行1：监听状态 · 端口 · 开始/停止 · **断开** · **隐藏** · **预览开** · 右侧 **本机 IP**（单击复制）
  - 行2：**连接** · fps · 最近帧大小 · 等比/拉伸 · **允许** · **设置**
- **中部**：黑底预览（默认等比铺满，`F11` 全屏）；空态副标题「本机 · 端口」；关预览时提示「预览已关闭」
- **保存栏两行**
  - 行1：保存开关 · 目录 · 浏览（默认目录 `%UserProfile%\Pictures\SF_view`）
  - 行2：**间隔（秒）** · **质量 1–100** · 最近写入
- **日志**：最近若干行

允许 IP：**设置** 弹窗标题「允许连接」；可选「全部放行」或勾选已知 IP（无手动添加/删除）。

## 图标

- 默认：深灰底 + 蓝灰「视窗」图标（嵌入 exe / 窗体 / 托盘）
- 自定义（重启生效）：`SF_view.exe` 同目录 `SF_view.ico` → `%AppData%\SF_view\app.ico` → 内嵌默认
